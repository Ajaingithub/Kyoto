...
library(stringr)
PBC_HCC_naive@meta.data$treatment_Resp <- PBC_HCC_naive@meta.data$cell_to_sample_ID
for (i in seq_along(patterns)) {
    pattern <- paste0("\\b", patterns[i], "\\b")
    PBC_HCC_naive@meta.data$treatment_Resp <- str_replace_all(PBC_HCC_naive@meta.data$treatment_Resp, pattern, replacements[i])
...
# $treatment_Resp
#                             diff         lwr         upr     p adj
# antiPD1_R-antiPD1_NR  0.09825421  0.08141828  0.11509014 0.0000000
# control-antiPD1_NR    0.10998464  0.08381658  0.13615270 0.0000000
# pbc-antiPD1_NR       -0.04892805 -0.07129111 -0.02656499 0.0000001
# control-antiPD1_R     0.01173043 -0.01772788  0.04118874 0.7359108

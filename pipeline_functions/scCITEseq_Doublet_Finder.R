doublet_scCITEseq <- function(Obj,dims,res,saveDir,Obj2,samplename,process,Assay){
  ...
  sweep.res.list <- paramSweep(GEX_obj_3, PCs = 1:dims, sct = FALSE) # sctranform = FALSE
  ...
  pK_choose <- as.numeric(as.character(pK_choose))
  nExp = as.numeric(nExp_poi)

  ## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
  GEX_obj_3 <- doubletFinder(GEX_obj_3, PCs = 1:dims, pN = 0.25, pK = pK_choose, 

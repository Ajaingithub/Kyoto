...
pseudobulk_within_cluster_AJ_no_batch_correction <- function(obj, savedir, group1, group2, grouping_by, cluster="all",
                                                             cluster_group = "seurat_clusters", cell_freq=10,
                                                             remove_samples=NULL, gene_min_counts = 5, sample_col = "orig.ident", splitting = "-"){
  ...
  if(cluster[1]=="all"){
    ...
    # Treg_cts_4 <- Treg_cts_2                              
    gene_symbols <- mapIds(
        org.Hs.eg.db,
        keys = rownames(Treg_cts_2),
        column = "SYMBOL",
        keytype = "ENSEMBL",
        multiVals = function(x) paste(x, collapse = ";")
        )
    
    Treg_cts_2$genenames  = as.data.frame(gene_symbols)[,1]
    Treg_cts_3 = Treg_cts_2[!duplicated(Treg_cts_2$genenames),]
    rownames(Treg_cts_3) <- Treg_cts_3$genenames
    Treg_cts_4 <- Treg_cts_3[,-ncol(Treg_cts_3)]

    if(length(sample_low_2) != 0){
      index <- grep(paste(sample_low_2,collapse="|",sep = ""),colnames(Treg_cts_4), invert=TRUE)
      Treg_cts_5 <- Treg_cts_4[,index]
    } else {
      Treg_cts_5 <- Treg_cts_4
    }
    
    print(dim(Treg_cts_5))
    Treg_metadata <- data.frame(samples = colnames(Treg_cts_5))
    ...
    colnames(Treg_metadata_2) <- c(sample_col,grouping_by)
    ...
    stopifnot(all(Treg_metadata_2$orig.ident == colnames(Treg_cts_5))) # if TRUE move forward
    ...
    design_AJ <- as.formula(paste("~",grouping_by,sep = ""))
    Treg_dds <- DESeqDataSetFromMatrix(countData = Treg_cts_5,
    ...
    plot_list[[1]] <-ggplot(pc_with_metadata, aes_string("PC1", "PC2", label="orig.ident", color=grouping_by)) +
    ...
    if(length(sample_low_2) != 0){
      index <- grep(paste(sample_low_2,collapse="|"),colnames(Treg_cts_4), invert=TRUE)
      Treg_cts_5 <- Treg_cts_4[,index]
    } else {
      Treg_cts_5 <- Treg_cts_4
    }
    ...
    df <- table(Treg_metadata_2[,grouping_by]) %>% as.data.frame()
    
    if(all(df$Freq >=3) == FALSE) {
    #   obj@meta.data[,paste(sample_col,batch_col,sep = "_")] <- paste(obj@meta.data[,sample_col],obj@meta.data[,batch_col],sep = "_")
    #   batch_group <- unique(obj@meta.data[,batch_col])
      # for (i in 1:length(batch_group)) {
      #   donor_change <- unique(obj@meta.data[,paste(sample_col,batch_col,sep = "_")]) %>% grep(batch_group[i],.,value=TRUE) %>% 
